...
This is the CANONICAL, package-owned collector. The runtime materializes it into a project's
.robotbase/ at launch (see Runtime._ensure_collector) and runs THAT — never the project's own
scripts/ copy. That keeps the collector's CLI contract (--output/--world/--robot) locked to the
runtime that launches it, so a project scaffolded from an older template can't run a stale collector
whose args mismatch (which crashes argparse and silently records nothing).

Runs inside the ROS container from sim launch until it is killed. It tracks the minimum LiDAR range
and collision/contact over the entire run (not just a trailing window), so `no_collision`/`no_contact`
truly mean "none occurred during the run".

Pose (final_x/y/yaw, distance, path) comes from the robot's GROUND-TRUTH world pose
(gz /world/<world>/dynamic_pose/info via gz-transport), NOT /odom: wheel odometry drifts (it doesn't
track a set_robot_pose teleport and integrates wheel-slip against walls), which would score
robot_reached_pose in the wrong frame. /odom is the pose fallback and the source of the reported
velocities. The runtime kills the collector at collect time and reads the JSON it leaves behind.
"""
import argparse
import json
import math
