...
from robotbase.robotspec.merge import build_model
from robotbase.robotspec.backends.urdf import render_urdf


def _render(name, root, fragments):
    return render_urdf(build_model(name, root, fragments))
...
def test_diff_drive_renders_expected_tokens():
    urdf = _render("warehouse_bot", "base_footprint", [differential_drive(_params(), None)])
...
def test_arm_renders_the_joint_chain():
    from robotbase.robotspec.modules import MODULES
    urdf = _render("arm_bot", "world", [MODULES["arm"]({}, None)])
...
def test_quadrotor_renders_body_and_rotors():
    from robotbase.robotspec.modules import MODULES
    urdf = _render("drone", "base_link", [MODULES["quadrotor"]({}, None)])
